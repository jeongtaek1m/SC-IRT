...
CONTROLS = {'lane': 'R2, lane graph kept', 'noroute': 'R2 w/o route relation',                 # 'lane' = the encoder earlier releases shipped
            ...
            'sslego': 'R2-noLane, track+ego SSL initialisation (masked agent tracks AND masked route ego steps, so the ego branch is pre-trained too) then the same IRT loss (initialisation ablation)',
            'match0.1_ssl': 'track-SSL initialisation + the difficulty-matching term lambda .1 in the IRT loss (both additions together)',
            'match0.1_sslego': 'track+ego SSL initialisation + the difficulty-matching term lambda .1 (both additions together)',
            'sslegoctx': 'track+ego SSL initialisation, ego head with a temporal context path (a hidden step reads its neighbours) then the same IRT loss (initialisation ablation)',
            'ssljepa': 'JEPA initialisation (EMA target encoder + predictor, latent targets, no input reconstruction) then the same IRT loss (initialisation ablation)',
