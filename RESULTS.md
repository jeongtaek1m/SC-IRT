...
| Agent density + kinematics | 0.736 | 0.200 | +0.191 |
...
`ens6 − GT:ck+gtrisk` is Δ = −0.005, CI [−0.146, +0.134] — it straddles zero, and
...
- **A2 — fixed probe-20 descriptor comparison**: the UP columns of
  `descriptor_table`, demonstrating that aggregate MAE saturates at .050–.057 for
  every descriptor and therefore cannot discriminate between them.
- **A3 — noise ceiling** (`noise_ceiling.py`): split-half 0.692 → reliability
  0.818 → **ceiling 0.904**.
- **A4 — SE calibration**: nominal tau 0.40 yields an empirical 0.35, and tau 0.35
  yields 0.33, on the 2PL calibrated bank. The 1PL bank is overconfident.
