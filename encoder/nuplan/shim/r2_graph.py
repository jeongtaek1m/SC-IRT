...
R2-noLane (LANE-FREE ENCODER) — `--ablate-lane`
  ...
  TRANSFER (this lineage only).  --ablate-lane is also accepted by --full-train --transfer-to,
  where it is applied to the SOURCE graph and to EVERY target graph through the same load_graph
  argument, so no domain can be lane-free by accident while another is not.  The output file
  gets the marker _nolane and records arm='r2nolane', ablate_lane=True.  Verified on both the
  b2d source and the nuplan target by verify_nolane_frozen.py.
...
def run_full(a):
    ...
    # --ablate-lane IS accepted here (unlike the correspondence controls): R2-noLane is an
    # architectural arm of the transfer itself, and it is applied to the SOURCE graph and to
    # EVERY target graph through the same load_graph argument, so no domain can end up lane-free
    ...
    if a.ablate_lane:                   # R2-noLane bookkeeping; absent under the defaults
        out.update(arm='r2nolane', ablate_lane=True)
...
def main():
    ...
    ap.add_argument('--ablate-lane', action='store_true',
                    help='R2-noLane: the LANE-FREE encoder. Removes the whole map side of the '
                         ...
                         'arm. In-domain it writes r2nolane_<domain>_s<seed>.npz; under '
                         '--full-train --transfer-to it is applied to the source AND to every '
                         'target graph and the output gets the marker _nolane. arm and '
                         'ablate_lane are recorded in the file either way.')
    ...
    a.zs_suffix = ((f'_tcol' if a.target == 'collision' else '')
                   ...
                   # R2-noLane marker: transfer runs only, so an in-domain --ablate-lane run
                   # keeps the r2nolane_<domain>_s<seed>.npz name and the zs_suffix assert below
                   + ('_nolane' if (a.ablate_lane and a.transfer_to) else ''))
    if a.zs_suffix:
