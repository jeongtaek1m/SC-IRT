...
| `run_av_baselines.py --merge` | the FST port, the GP-adaptive port, the official-code GP run, kernel test case sampling, the DICE re-implementation and the official Sim2Val control-variates estimator on the Table 1 protocol (`results/up_avbase.json`): SR-MAE, the paired delta against ATDrive and the pairwise ranking accuracy of every cell (random-draw rows: per draw, then averaged); no anchors |
...
for lo in 0 2 4 6 8 10 12 14; do python experiments/run_av_baselines.py --methods ktcs_scene --seeds $lo $((lo+2)) & done; wait
CUDA_VISIBLE_DEVICES=1 python experiments/dice_mae.py                 # the bank-trained MAE embedding (data/features/eval_dice_mae.npz, ~1 min)
for lo in 0 2 4 6 8 10 12 14; do python experiments/run_av_baselines.py --methods dice_full dice_mae dice_desc --seeds $lo $((lo+2)) & done; wait
