...
| `run_av_baselines.py --merge` | the FST port, the GP-adaptive port, the official-code GP run and kernel test case sampling on the Table 1 protocol (`results/up_avbase.json`): SR-MAE, the paired delta against ATDrive and the pairwise ranking accuracy of every cell; no anchors |
