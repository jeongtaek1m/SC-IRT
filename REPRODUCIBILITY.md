...
| `run_av_baselines.py --merge` | the FST port, the GP-adaptive port, the official-code GP run, kernel test case sampling and DICE-style sampling on the Table 1 protocol (`results/up_avbase.json`): SR-MAE, the paired delta against ATDrive and the pairwise ranking accuracy of every cell; no anchors |
...
for lo in 0 2 4 6 8 10 12 14; do python experiments/run_av_baselines.py --methods dice_desc dice_mae --seeds $lo $((lo+2)) & done; wait
