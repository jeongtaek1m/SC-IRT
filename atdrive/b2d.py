"""B2D data loading for the ATDrive protocol.
...
def load_response_rows(csv_path=None):
    ...
    rows = list(csv.reader(open(csv_path or os.environ.get('ATDRIVE_RESPONSE_CSV', DEFAULT_CSV))))
    rids = rows[0][1:]
...
class Panel:
    ...
    family, spread over the ability range; `csv_path` or ATDRIVE_RESPONSE_CSV
